# Scraperr Enhanced Web Application with Authentication

This is an advanced web interface for interacting with the Scraperr API with authentication and content type selection. The application provides a modern, user-friendly interface with enhanced capabilities for managing all scraping tasks and configurations.

## Features

- **Authentication System**: Secure login/logout functionality to protect access
...
2. Open the authenticated web application in your browser:
   ```bash
   xdg-open web-app/index-auth.html
   ```
   
   Or simply open the file directly in your browser by navigating to:
   `file:///home/ncx/Proyectos/scraperr-local/web-app/index-auth.html`

### Authentication

1. On first load, you'll see the authentication screen
2. Enter your username and password (in this demo version, any non-empty values work)
3. Click "Iniciar Sesión" to access the application
4. To log out, click the "Cerrar Sesión" button in the "Jobs & Docs rápidos" section
